...
## See Also
If you need more than v1 can express, there's also a **v2** format. Instead of a `speed` per chunk, v2 uses an explicit `tb` (timebase) and a list of `clips` that reference named `effects`, so you can apply actions beyond just changing speed. It's still a single-source format. You can generate one with `auto-editor example.mp4 --export v2`.
