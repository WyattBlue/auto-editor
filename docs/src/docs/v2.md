...
Like v1, v2 only supports a single source and a "linear" timeline: sections must be laid down in order, and sections later in the source cannot be placed ahead of earlier ones. Like v1, it cannot represent [transitions](./transition); exporting drops them and keeps the cuts.
