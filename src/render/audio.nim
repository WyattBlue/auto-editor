...
proc createAudioFilterGraph(effect: Action, sr: int, layout: string): (ptr AVFilterGraph,
...
proc processAudioClip(tl: v3, clip: Clip, data: seq[int16], sourceSr: cint, targetSr: cint): seq[int16] =
  ...
  if needsFiltering:
    ...
    let layout = "stereo"
    let (filterGraph, bufferSrc, bufferSink) = createAudioFilterGraph(effect, sourceSr, layout)
    defer:
      if filterGraph != nil:
