...
def kdenlive_write(output: str, tl: v3) -> None:
    ...
    for clip_idx in warped_clips:
        for i in range(a_channels + v_channels):
            ...
            if not path in source_ids:
                source_ids[path] = str(source_id)
                source_id += 1
            
            prod = ET.SubElement(
                mlt, "producer", attrib={"id": f"producer{producers}", "in": "00:00:00.000", "out": global_out}
            )
            ET.SubElement(prod, "property", name="resource").text = f"{clip.speed}:{path}"                
            ...
            if i < a_channels:
                ...
                ET.SubElement(prod, "property", name="astream").text = str(a_channels-1-i)
                ET.SubElement(prod, "property", name="set.test_audio").text = "0"
                ET.SubElement(prod, "property", name="set.test_video").text = "1"
            else:
                ET.SubElement(prod, "property", name="vstream").text = str(v_channels-1-(i-a_channels))
    ...
    for i, audio in enumerate(tl.a):
        ...
        ET.SubElement(tractor, "track", attrib={"hide": "video", "producer": f"playlist{playlists-2}"})
        ET.SubElement(tractor, "track", attrib={"hide": "video", "producer": f"playlist{playlists-1}"})
        chains += 1
    
    # create chains, playlists and tractors for video channels
    for i, video in enumerate(tl.v):
        path = str(video[0].src.path)
        ...
        ET.SubElement(tractor, "property", name="kdenlive:timeline_active").text = "1"
        ET.SubElement(tractor, "track", attrib={"hide": "audio", "producer": f"playlist{playlists-2}"})
        ET.SubElement(tractor, "track", attrib={"hide": "audio", "producer": f"playlist{playlists-1}"})
        chains += 1
    ...
    for clip in clips:
        ...
        for i, playlist in enumerate(clip_playlists[::2]):
            ...
            group["children"].append({"data": f"{i}:{clip.start+group_counter}", "leaf": "clip", "type": "Leaf"})
            clip_prod = ""
            ...
            entry = ET.SubElement(playlist, "entry", attrib={"producer": f"{clip_prod}", "in": _in, "out": _out})
    ...
    ET.SubElement(sequence, "property", name="kdenlive:sequenceproperties.groups").text = json.dumps(groups, indent=4)
    ...
    ET.SubElement(playlist_bin, "property", name="kdenlive:docproperties.uuid").text = f"{{{seq_uuid}}}"
    ET.SubElement(playlist_bin, "property", name="kdenlive:docproperties.version").text = "1.1"
